...
def generate_para():
    ...
    opt.input_dim = 1 # input sequence dimension
    opt.output_dim = 1 # output sequence dimension
    opt.hidden_dim = 128 # hidden state dimension
...
class LSTM_model(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers, DEVICE):
        super().__init__()
...
def generate_LSTM_model(opt, teacher=False):
    ...
    model = LSTM_model(opt.input_dim, opt.output_dim, opt.hidden_dim, opt.num_layers, opt.DEVICE)
...
class DANNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.sharedNet = generate_LSTM_model(opt) # shared LSTM network
        self.bottleneck_1 = generate_BT_model(opt) # sepearted branch, tailored net
        self.bottleneck_2 = generate_BT_model(opt) # sepearted branch, tailored net

        self.linear_1 = nn.Linear(opt.hidden_dim, opt.output_dim) # fully connected layer at last
        self.linear_2 = nn.Linear(opt.hidden_dim, opt.output_dim) # fully connected layer at last
